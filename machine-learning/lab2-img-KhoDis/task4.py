...
(train_images_fashion, train_labels_fashion), (test_images_fashion, test_labels_fashion) = keras.datasets.fashion_mnist.load_data()
...
fashion_model = keras.models.Sequential([
    keras.layers.Conv2D(32, (3, 3), activation='relu', input_shape=(28, 28, 1), weights=mnist_model.layers[0].get_weights(), trainable=False),
    keras.layers.MaxPooling2D((2, 2)),
    keras.layers.Conv2D(64, (3, 3), activation='relu', weights=mnist_model.layers[2].get_weights(), trainable=False),
    keras.layers.MaxPooling2D((2, 2)),
    keras.layers.Conv2D(64, (3, 3), activation='relu', weights=mnist_model.layers[4].get_weights(), trainable=False),
...
fashion_model.save('task4/fashion_mnist_model.h5')
...
print("Lengths for history:", len(train_losses), len(test_losses), len(train_accuracies), len(test_accuracies))
...
plt.legend()
plt.savefig('task4/learning_curve.png')
plt.clf()
...
print("Lengths for plotCallback:", len(train_losses), len(test_losses), len(train_accuracies), len(test_accuracies))
...
plt.legend()
plt.savefig('task4/learning_curve_plot_callback.png')
plt.clf()
